...
export default function KanjiCanvas({ lineWidth, color, clear }: KanjiCanvasProps) {
    ...
    // draw() now only collects points — no rendering
    const draw = (e: React.PointerEvent<HTMLCanvasElement>) => {
        if (!isDrawing.current) return;
    ...
    useEffect(() => {
        ...
        const render = () => {
            if (isDrawing.current && points.current.length >= 4) {
                const [p0, p1, p2, p3] = points.current.slice(-4);
    ...
    useEffect(() => {
        ...
        const preventDefault = (e: TouchEvent) => e.preventDefault();
        canvasCurrent.addEventListener("touchstart", preventDefault, { passive: false });
        canvasCurrent.addEventListener("touchmove", preventDefault, { passive: false });
